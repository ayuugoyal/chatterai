...
import { Code, Copy, ExternalLink, MessageSquare, RefreshCw, Save, Loader2 } from "lucide-react"
...
import { z } from "zod"
import { getAgentConversations, getConversationMessages } from "@/lib/actions/conversation-actions"
import { updateAgent, AgentFormValues } from "@/lib/actions/agent-actions"
...
const AgentTabs = (agentData: AgentTable) => {
  ...
  const form = useForm<z.infer<typeof configSchema>>({
    ...
    defaultValues: {
      ...
      urls: [],
      saveConversations: agentData.saveConversations ?? true,
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof configSchema>) => {
    setIsSaving(true);
    try {
      const result = await updateAgent(agentData.id, values as AgentFormValues);
  ...
  const fetchConversations = async () => {
    ...
    const convos = await getAgentConversations(agentData.id);
  ...
  return (
    <Tabs defaultValue="embed" className="mt-6">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Update your agent&apos;s name, slug, model provider, and knowledge base
                </CardDescription>
              </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. support-bot" className="font-mono" {...field} />
                        </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <div className="grid grid-cols-3 gap-4 mt-2">
                        {[
                          ...
                          <div
                            ...
                            className={`
                              cursor-pointer rounded-lg border-2 p-4 flex flex-col items-center justify-center gap-2 transition-all duration-200
                      ...
                      <FormDescription>Choose the AI model to power your agent</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="password" placeholder="Your Gemini API key" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="password" placeholder="Your Anthropic API key" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="password" placeholder="Your OpenAI API key" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Define your agent&apos;s personality and behavior</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        <FormLabel>Save Conversations</FormLabel>
                        <FormDescription>Store user conversations for analytics</FormDescription>
                      </div>
                      <FormControl>
                ...
                <div className="bg-muted p-2 rounded-md text-sm">
                  {agentData.isActive ? (
                    ...
                    <span className="text-red-600 font-medium flex items-center gap-1">
                      <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M5.5 1C5.22386 1 5 1.22386 5 1.5C5 1.77614 5.22386 2 5.5 2H9.5C9.77614 2 10 1.77614 10 1.5C10 1.22386 9.77614 1 9.5 1H5.5ZM3 3.5C3 3.22386 3.22386 3 3.5 3H5H10H11.5C11.7761 3 12 3.22386 12 3.77614 11.7761 4 11.5 4H11V12C11 12.5523 10.5523 13 10 13H5C4.44772 13 4 12.5523 4 12V4H3.5C3.22386 4 3 3.77614 3 3.5ZM5 4H10V12H5V4Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd"></path>
                      </svg>
                      Inactive
                    </span>
                  )}
      ...
      <TabsContent value="conversations" className="h-[600px]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
          <Card className="md:col-span-1 h-full overflow-hidden flex flex-col">
            <CardHeader className="py-4 px-4 bg-muted/20 border-b flex flex-row justify-between items-center">
          ...
          <Card className="md:col-span-2 h-full overflow-hidden flex flex-col">
            <CardHeader className="py-4 px-4 bg-muted/20 border-b flex flex-row justify-between items-center">
              <CardTitle className="text-sm">
                {selectedConversationId ? "Message History" : "Select a Conversation"}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0 flex-1 overflow-auto">
              {isLoadingMessages ? (
