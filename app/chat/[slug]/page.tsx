...
export default function ChatPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    if (!storedSessionId) {
      // Generate new sessionId using crypto.randomUUID or fallback
  ...
  useEffect(() => {
    const fetchAgentDetails = async () => {
      try {
          ...
          try {
            ...
            if (response.ok) {
              ...
              const mergedConfig = {
                ...
                headerTitle: config.headerTitle || agentData.name || "Chat Support",
              ...
              if (mergedConfig.welcomeMessage) {
                ...
                // Update the welcome message in the chat if it's the first message
        ...
        setAgent(
          { name: "AI Assistant" } as AgentTable,
        )
      } finally {
        ...
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 text-xs font-medium" style={{ color: uiConfig.textColor, opacity: 0.7 }}>
          ...
          <AnimatePresence initial={false}>
            {groupedMessages.map((group, groupIndex) => (
              ...
              >
                ...
                <div
                  className={cn("flex flex-col gap-1.5", group.role === "user" ? "items-end" : "items-start w-full")}
                >
                ...
                {group.role === "user" && (
                  <Avatar className="h-8 w-8 mt-4 flex-shrink-0 shadow-sm border border-white dark:border-black">
                    <AvatarFallback className="bg-indigo-100 text-indigo-600 dark:bg-indigo-900 dark:text-indigo-200 text-xs font-bold">You</AvatarFallback>
                  </Avatar>
                )}
              </motion.div>
            ))}
